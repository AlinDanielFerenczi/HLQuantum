...
class Circuit:
    ...
    def rx(self, target: int, angle: float) -> "Circuit":
        self._validate_qubits(target)
        self.gates.append(Gate(name="rx", targets=(target,), params=(angle,)))
        return self

    def ry(self, target: int, angle: float) -> "Circuit":
        self._validate_qubits(target)
        self.gates.append(Gate(name="ry", targets=(target,), params=(angle,)))
        return self

    def rz(self, target: int, angle: float) -> "Circuit":
        self._validate_qubits(target)
        self.gates.append(Gate(name="rz", targets=(target,), params=(angle,)))
        return self
